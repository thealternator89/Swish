...
export = {
    ...
    description:
        'Decodes a JWT and makes it human-readable',
    id: 'decode-jwt',
    author: 'thealternator89',
    beepVersion: '1.0.0',
    process: async (args: PluginArgument) => {
        const [headerB64, payloadB64, signature] = args.textContent.split('.');
        const [header, payload] = [headerB64, payloadB64].map(decodeBase64).map((str) => JSON.parse(str));
...
function indentLines(text: string, spaces: number): string {
    return text.split('\n').map((line) => whitespace(spaces) + line).join('\n');
}
