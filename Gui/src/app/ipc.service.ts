...
export class IpcService {
  ...
  constructor() {
    try {
      ...
      console.error('Failed to load IPC. Communication with host process will fail.')
    }
  }

  registerMenuCommands(): Observable<string> {
    return new Observable((observer) => {
      this.ipc.on(IPC_CHANNELS.MENU_COMMAND, (_event, arg) => observer.next(arg));
    });
  }

  registerPluginStatusUpdates(): Observable<{status: string, id: string}> {
    return new Observable((observer) => {
      this.ipc.on(IPC_CHANNELS.PLUGIN_STATUS_UPDATE, (_event, arg) => observer.next(arg));
    });
  }

  registerPluginProgressUpdates(): Observable<{percentage: number, id: string}> {
    return new Observable((observer) => {
      this.ipc.on(IPC_CHANNELS.PLUGIN_PROGRESS_UPDATE, (_event, arg) => observer.next(arg));
    });
  }
  ...
  runPlugin(runPluginRequest: RunPluginRequest): Promise<RunPluginResponse> {
    ...
    return new Promise((resolve, reject) => {
      this.ipc.once(IPC_CHANNELS.RUN_PLUGIN.RES, (_event, arg: RunPluginResponse) => {
        resolve(arg);
      });
    })
  }
